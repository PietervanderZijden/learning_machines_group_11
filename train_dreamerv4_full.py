...
Phases follow Algorithm 1:
1. causal tokenizer pretraining,
2. shortcut world-model pretraining,
3. joint world-model + task-agent MTP finetuning,
4. PMPO imagination training.
"""
...
def sample_latent_batch(
    ...
    for _ in range(batch_size):
        ...
        if random.random() < start_frame_fraction:
            ...
            # _world_sequence() reads arrival latents from positions 1..T.
...
def log_tokenizer_reconstructions(agent, refs, wandb_run, num_samples=4):
    ...
    try:
        ...
        for ref in sampled_refs:
            ...
            # Take the middle frame of each sampled episode
            mid = length // 2
