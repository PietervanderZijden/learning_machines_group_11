...
def test_tokenizer_masking_does_not_explode_gradients():
    ...
    divide by sqrt(eps) and produced gradient norms in the billions.
    """
    ...
    # The mask token itself should not dominate the gradient budget.
...
def test_tokenizer_mse_is_computed_only_on_masked_patches():
    ...
    # Deterministic forward pass to get the reported masked MSE.
    torch.manual_seed(42)
    output = agent.tokenizer(images, ir)
    reconstruction = output["reconstruction"]
    reported_mse = output["mse_loss"].item()

    # Recompute the same patch mask by replaying the RNG state.
    torch.manual_seed(42)
    ...
    # The full-pixel MSE should differ (with high probability) because
    # unmasked pixels are included there but excluded from the objective.
...
def test_tokenizer_grad_norm_does_not_grow_progressively():
    ...
    With full-pixel MSE the loss collapses as unmasked reconstruction
    improves, RMS normalization rescales gradients up, and tok/grad_norm
    grows monotonically. The masked-only objective removes the collapse,
    so grad_norm stays bounded and does not trend upward.
    """
    agent = DreamerV4FullAgent(small_config(lpips_weight=0.0), "cpu")
    images = torch.rand(2, 3, 3, 16, 16)
    ir = torch.rand(2, 3, 2)
...
def test_qk_normalization_does_not_use_sqrt_scaling():
    ...
    additional sqrt(head_dim) factor that over-amplifies attention logits.
    """
...
def test_running_rms_floor_prevents_runaway_amplification():
    ...
    fraction of the maximum observed value, preventing gradient amplification."""
    ...
    # Use a fast-decaying RMS so the EMA reaches the floor in few steps.
    for _ in range(20):
        normalized = rms.normalize(torch.tensor(0.001))
    # Divisor is floored at 0.1 * max_value = 0.1, so normalized = 0.001 / 0.1.
...
def test_dynamics_unknown_actions_do_not_explode_gradients():
    ...
    divide by sqrt(eps) and produced gradient norms in the hundreds of billions.
    """
    agent = DreamerV4FullAgent(small_config(), "cpu")
    cfg = agent.cfg
