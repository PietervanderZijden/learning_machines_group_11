...
class GITWorldModel(nn.Module):
    ...
    def forward(self, data, type):
        ### what an unremarkable code
        if type == "regular":
            obs, action, reward, termination = data["obs"], data["action"], data["reward"], data["termination"]
            self.train()
            batch_size, batch_length = obs.shape[:2]
            with torch.autocast(device_type='cuda', dtype=torch.float16, enabled=self.use_amp):
                embedding = self.encoder(obs)
            ...
            return total_loss, reconstruction_loss, reward_loss, termination_loss, dynamics_loss, representation_loss, dynamics_real_kl_div, representation_real_kl_div
        elif type == "encode_obs":
            obs = data["sample_obs"]
            with torch.autocast(device_type=self.device.type, dtype=torch.float16, enabled=self.use_amp):
                embedding = self.encoder(obs)
                post_logits = self.dist_head.forward_post(embedding)
    ...
    def update(self, obs, action, reward, termination, logger=None):
        total_loss, reconstruction_loss, reward_loss, termination_loss, dynamics_loss, representation_loss, dynamics_real_kl_div, representation_real_kl_div = self.forward(obs, action, reward, termination, logger)
