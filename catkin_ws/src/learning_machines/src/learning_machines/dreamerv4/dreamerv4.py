...
NOTE: This is NOT DreamerV4 as described in the paper. It is a simpler
transformer-based world model with Dreamer-style actor-critic. The actual
DreamerV4 implementation (with shortcut forcing, PMPO, etc.) is in
dreamerv4_image.py.

Training:
  1. World model: predict next obs, reward, done from (obs, action, reward) sequences
  2. Actor-critic: imagine future rollouts with world model, optimize policy via REINFORCE
"""
...
class TransformerDreamerAgent(nn.Module):
    ...
    This is NOT DreamerV4 (which uses shortcut forcing, PMPO, etc.).
    It is a simpler autoregressive transformer world model for vector observations.

    Uses Dreamer-style state-value critic (not SAC-style Q-function).
    Uses two-hot distributional loss for critic.
    Uses return normalization.
    """
    ...
    def __init__(self, cfg: DreamerV4Config):
        ...
        # Dreamer-style: state-value critic V(s), not Q(s,a)
    ...
    def warmup_stats(self, buffer):
        ...
        # Apply symlog before computing reward stats for better normalization
    ...
    def update_world_model(self, batch: dict[str, torch.Tensor]) -> dict[str, float]:
        ...
        # Apply symlog first, then normalize (matches warmup_stats)
        ...
        # Obs reconstruction loss in normalized space
        ...
        # Reward prediction loss (MSE on normalized symlog rewards)
    ...
    def update_actor_critic(self, batch: dict[str, torch.Tensor]) -> dict[str, float]:
        ...
        imag_obs = imagined["observations"]  # (B, H+1, obs_dim) in raw space
        imag_rew = imagined["rewards"]  # (B, H) in raw space
        imag_done = imagined["dones"]  # (B, H)
        imag_log_prob = imagined["log_probs"]  # (B, H, 1)

        B, H = imag_rew.shape

        # Use observations after each action (positions 1..H), not the initial obs
        imag_obs_after = imag_obs[:, 1:H + 1]  # (B, H, obs_dim)
        imag_obs_norm = self.obs_stats.normalize_tensor(imag_obs_after)
        # Clamp to prevent extreme values from out-of-distribution imagined observations
        imag_obs_norm = torch.clamp(imag_obs_norm, -5.0, 5.0)

        # Get critic values (distributional logits)
        ...
        with torch.no_grad():
            # Bootstrap from the last imagined state
        ...
        # Reshape returns to match logits shape
        ...
        # Paper: L(θ) = -Σ sg((R_t^λ - v_t) / max(1, S)) * log π(a_t|s_t) + η H[π]
        with torch.no_grad():
        ...
        # H[pi] ~= -log pi(a|s), so minimizing -eta*H adds eta*log_prob.
...
# Backward-compatible canonical alias. Keep TransformerDreamerAgent available as
# a legacy vector prototype, but direct DreamerV4Agent imports use the image
# shortcut-forcing implementation.
