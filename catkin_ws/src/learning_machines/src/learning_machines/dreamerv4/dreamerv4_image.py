...
def _sample_tau_and_d(
    ...
    num_levels = int(np.log2(k_max)) + 1
    d_indices = torch.randint(0, num_levels, (batch_size,), device=device)
    d = 1.0 / (2.0 ** d_indices.float())



    num_steps = (1.0 / d).long()
...
class ImageDynamicsModel(nn.Module):
    ...
    def __init__(
        ...
        self.tau_embed = nn.Embedding(8, d_model)
        self.d_embed = nn.Embedding(8, d_model)
    ...
    def imagine(
        ...
        return {
            ...
            "reward_logits": torch.stack(rew_logits_list, dim=1),
            "dones": torch.cat(done_list, dim=1),
...
class ImageDreamerV4Agent(nn.Module):
    ...
    def update_dynamics(
        ...
        z0 = torch.randn_like(latents[:, 1:])
        z1 = latents[:, 1:]


        tau_expanded = tau.unsqueeze(-1)
        z_tilde = (1 - tau_expanded) * z0 + tau_expanded * z1
        ...
        z1_pred = preds["latent_pred"]


        d_min = 1.0 / _K_INFERENCE
        is_flow = (d <= d_min + 1e-6)
        is_bootstrap = ~is_flow


        flow_loss = F.mse_loss(z1_pred, z1, reduction="none")
        flow_loss = flow_loss.mean(-1)
        ...
        if active_bootstrap.any():
            ...
            if len(bootstrap_idx[0]) > 0:
                ...
                tau_b = tau[bootstrap_idx]
                d_b = d[bootstrap_idx]
                z_tilde_b = z_tilde[bootstrap_idx]
                z1_b = z1[bootstrap_idx]
                actions_b = actions[bootstrap_idx]
                ...
                bootstrap_loss_sample = bootstrap_loss_sample.mean(-1)

                bootstrap_loss[bootstrap_idx] = bootstrap_loss_sample


        weights = _ramp_weight(tau)
        ...
        rew_logits = preds["reward_logits"]
    ...
    def update_actor_critic(
        ...
        imag_latents = imagined["latents"]
        imag_rew_logits = imagined["reward_logits"]
        imag_done_logits = imagined["dones"]
        ...
        imag_rewards = logits_to_value(imag_rew_logits)
        ...
        advantages = (returns - imag_values).detach()
        ...
        if pos_mask.sum() > 0:
            pos_loss = -log_prob_flat[pos_mask].mean()
        else:
            pos_loss = torch.tensor(0.0, device=adv_flat.device)

        if neg_mask.sum() > 0:
            neg_loss = +log_prob_flat[neg_mask].mean()
        else:
