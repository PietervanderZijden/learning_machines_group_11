...
def red_green_saliency_loss(
    prediction: torch.Tensor, target: torch.Tensor
) -> torch.Tensor:
    """Preserve red block and green goal regions that global image MSE can ignore."""
    pred_red = torch.relu(
        prediction[..., 0, :, :]
        - torch.maximum(prediction[..., 1, :, :], prediction[..., 2, :, :])
    )
    pred_green = torch.relu(
        prediction[..., 1, :, :]
        - torch.maximum(prediction[..., 0, :, :], prediction[..., 2, :, :])
    )
    target_red = torch.relu(
        target[..., 0, :, :]
        - torch.maximum(target[..., 1, :, :], target[..., 2, :, :])
    )
    target_green = torch.relu(
        target[..., 1, :, :]
        - torch.maximum(target[..., 0, :, :], target[..., 2, :, :])
    )
    target_saliency = torch.maximum(target_red, target_green)
    pred_saliency = torch.maximum(pred_red, pred_green)
    weights = 1.0 + 20.0 * (target_saliency > 0.1).float()
    return ((pred_saliency - target_saliency).square() * weights).sum() / weights.sum()


green_saliency_loss = red_green_saliency_loss
...
class DreamerV3:
    ...
    def __init__(self, cfg: DreamerV3Config, device: str = "auto"):
        ...
        self.buffer = ReplayBuffer(
            ...
            online_queue_capacity=cfg.online_queue_capacity,
            store_latent_states=cfg.store_latent_states,
        )
