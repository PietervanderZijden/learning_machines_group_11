...
from .layers import RMSNormSiLU


class BlockGRUCell(nn.Module):
    """Independent GRU cells over deterministic-state blocks."""

    def __init__(self, input_size: int, hidden_size: int, blocks: int = 8):
        super().__init__()
        if hidden_size % blocks != 0:
            raise ValueError("hidden_size must be divisible by block count")
        if input_size % blocks != 0:
            raise ValueError("input_size must be divisible by block count")
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.blocks = blocks
        self.input_block = input_size // blocks
        self.hidden_block = hidden_size // blocks
        self.cells = nn.ModuleList(
            nn.GRUCell(self.input_block, self.hidden_block)
            for _ in range(blocks)
        )

    def forward(self, x: torch.Tensor, h: torch.Tensor) -> torch.Tensor:
        xs = x.split(self.input_block, dim=-1)
        hs = h.split(self.hidden_block, dim=-1)
        return torch.cat(
            [cell(x_block, h_block) for cell, x_block, h_block in zip(self.cells, xs, hs)],
            dim=-1,
        )
...
class RSSM(nn.Module):
    ...
    def __init__(
        ...
        self.block_gru_blocks = block_gru_blocks
        ...
        self.h_embed = nn.Linear(deterministic_size, hidden_size)
        self.dynamics_norm = RMSNormSiLU(hidden_size)

        # Paper-style Block GRU with mixed latent, action, and recurrent inputs.
        self.gru = BlockGRUCell(hidden_size, deterministic_size, block_gru_blocks)

        # Prior: p(z_t | h_t)
        self.prior_net = nn.Sequential(
            nn.Linear(deterministic_size, hidden_size),
            RMSNormSiLU(hidden_size),
        ...
        # RMSNorm for stability (only used for raw vector obs)
        if not obs_is_embedding:
            self.obs_norm = RMSNormSiLU(obs_dim)
    ...
    def observe(
        ...
        h_embed = F.silu(self.h_embed(h))

        # Block GRU dynamics with mixed input from latent, action, and h_{t-1}.
        gru_input = self.dynamics_norm(z_embed + act_embed + h_embed)
