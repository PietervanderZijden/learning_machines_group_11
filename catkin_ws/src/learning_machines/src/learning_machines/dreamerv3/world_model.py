...
from .layers import RMSNorm, RMSNormSiLU
...
def mlp(in_dim: int, hidden: int, out_dim: int, layers: int = 3) -> nn.Sequential:
    ...
    for i in range(len(dims) - 1):
        ...
        if i < len(dims) - 2:
            net.append(RMSNormSiLU(dims[i + 1]))
...
class CNNEncoder(nn.Module):
    ...
    def __init__(self, embed_dim: int = 512, image_size: int = 64, base_channels: int = 16):
        super().__init__()
        c1 = base_channels
        c2 = base_channels * 2
        c3 = base_channels * 4
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(3, c1, 4, stride=2, padding=1),  # 64->32
            nn.SiLU(),
            nn.Conv2d(c1, c2, 4, stride=2, padding=1),  # 32->16
            nn.SiLU(),
            nn.Conv2d(c2, c3, 4, stride=2, padding=1),  # 16->8
            nn.SiLU(),
            nn.Conv2d(c3, c4, 4, stride=2, padding=1),  # 8->4
            nn.SiLU(),
            nn.Flatten(),
            nn.Linear(c4 * 4 * 4, embed_dim),
            RMSNorm(embed_dim),
        )
...
class CNNDecoder(nn.Module):
    ...
    def __init__(self, state_dim: int = 512, image_size: int = 64, base_channels: int = 16):
        super().__init__()
        c1 = base_channels
        c2 = base_channels * 2
        c3 = base_channels * 4
        ...
        self.fc = nn.Sequential(
            nn.Linear(state_dim, c4 * 4 * 4),
            nn.SiLU(),
        )
        self.deconv = nn.Sequential(
            nn.ConvTranspose2d(c4, c3, 4, stride=2, padding=1),  # 4->8
            nn.SiLU(),
            nn.ConvTranspose2d(c3, c2, 4, stride=2, padding=1),  # 8->16
            nn.SiLU(),
            nn.ConvTranspose2d(c2, c1, 4, stride=2, padding=1),  # 16->32
            nn.SiLU(),
            nn.ConvTranspose2d(c1, 3, 4, stride=2, padding=1),  # 32->64
            nn.Sigmoid(),
        )
...
class WorldModel(nn.Module):
    ...
    def __init__(
        ...
        self.rssm = RSSM(
            ...
            block_gru_blocks=block_gru_blocks,
        ...
        if use_multimodal:
            ...
            self.ir_encoder = nn.Sequential(
                ...
                RMSNormSiLU(embed_size // 4),
                nn.Linear(embed_size // 4, embed_size // 2),
                RMSNorm(embed_size // 2),
            )
            self.obs_decoder = CNNDecoder(
        ...
        # Layer norms
        self.reward_norm = RMSNorm(mlp_hidden)
