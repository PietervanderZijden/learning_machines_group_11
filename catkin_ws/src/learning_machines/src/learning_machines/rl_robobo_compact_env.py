...
class RoboboCompactEnv(gym.Env):
    """Run compact food-collection or pushing transitions."""
    ...
    def _settle(self, seconds: float) -> None:
        """Advance the environment while physics and scripts settle."""
        if self._is_simulation:
            if seconds <= 0:
                return
    ...
    def _wait_for_tilt_script_ready(self, timeout: float) -> int | None:
        """Wait for the tilt motor script to initialize."""
        if not self._is_simulation:
