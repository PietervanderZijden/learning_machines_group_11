...
class InteractiveDynamics(nn.Module):
    ...
    def __init__(self, cfg: DreamerV4FullConfig):
        ...
        # which causes exploding gradients when actions are unknown.
        ...
        # Initialized away from zero to avoid RMSNorm gradient blow-up when
        # task embeddings are small or a single task is used.
    ...
    def _spatial_mask(self, device: torch.device) -> torch.Tensor:
        ...
        # Agent/task information may read the world-model stream, but cannot
        # influence any world-model token, preventing causal task confusion.
