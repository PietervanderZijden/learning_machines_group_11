...
class RunningRMS(nn.Module):
    def __init__(self, decay: float, floor_ratio: float = 0.1):
        ...
        # Start at 1.0 for a soft first step; max_value will be set from the
        # first observed loss and the floor will prevent runaway amplification
        # as losses shrink later.
    ...
    def normalize(self, loss: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():
            ...
            if not self.initialized:
                # Keep the original soft-start behavior (value=1.0) unless the
                # first loss is already larger, then track the peak for flooring.
...
class LPIPSLoss(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        # Keep the frozen external metric outside this module's state dict.
        # It is reproducibly loaded from the lpips package and would otherwise
        # add hundreds of megabytes to every agent checkpoint.
...
class CausalVideoTokenizer(nn.Module):
    ...
    def __init__(self, cfg: DreamerV4FullConfig):
        ...
        # Initialized away from zero: RMSNorm on zero inputs divides by sqrt(eps),
        # which causes exploding gradients for masked patches.
    ...
    def forward(
        ...
        # encode() squeezes time for single-frame inputs; restore it for loss masks.
        if patch_mask.dim() == 2:
            patch_mask = patch_mask.unsqueeze(1)
        reconstruction, ir_prediction = self.decode(latent)

        # Expand the patch mask to pixel resolution for masked-only MSE/LPIPS.
        # patch_mask: (B, T, P) bool, True = masked patch.
        ...
        if self.cfg.lpips_weight:
            # Hybrid LPIPS: reconstruction at masked positions, ground truth
            # at unmasked positions. Gradients flow only through masked regions.
            hybrid = torch.where(
