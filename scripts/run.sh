...
docker buildx build --tag learning_machines .
# Mounting to a directory that does not exist creates it.
# Mounting to relative paths works since docker engine 23
docker run -t --rm --name robobo-container -p 45100:45100 -p 45101:45101 -v "$(pwd)/results:/root/results" --env-file .env learning_machines "$@"
